  ...
  return (
    <>
      ...
      {((missed_commitments && missed_commitments.length > 0) || (relationship_risk_companies && relationship_risk_companies.length > 0)) && (
        <div className="card" style={{ borderLeft: '4px solid #EF4444', backgroundColor: 'var(--danger-bg)', marginBottom: '0' }}>
          <div className="card-header" style={{ borderBottomColor: 'var(--danger-border)', padding: '10px 16px', backgroundColor: 'var(--danger-bg)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--danger)', fontWeight: 700, fontSize: '12px' }}>
              <Bell size={14} />
              <span>MORNING BRIEF — COMMITMENT & RELATIONSHIP ALERTS ({(missed_commitments?.length || 0) + (relationship_risk_companies?.length || 0)})</span>
            </div>
            <span style={{ fontSize: '10px', color: 'var(--text-disabled)' }}>SCM Date: {formatDate(system_date)}</span>
          </div>
          <div className="card-body" style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {(missed_commitments || []).map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', padding: '8px 10px', background: 'var(--danger-bg)', borderRadius: '4px', border: '1px solid #FECACA' }}>
                ...
                <button className="btn btn-secondary" style={{ padding: '3px 8px', fontSize: '10px', borderColor: 'var(--danger-border)', color: 'var(--danger)', flexShrink: 0 }} onClick={() => onNavigate('commitment-health')}>
                  View Health
                </button>
              </div>
            ))}
            {(relationship_risk_companies || []).map((c, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', padding: '8px 10px', background: 'var(--warning-bg)', borderRadius: '4px', border: '1px solid #FDE68A' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      {(unconfirmed_snapshots_count > 0 || shortage_alerts.length > 0 || anomalous_pos.length > 0) && (
        <div className="card" style={{ borderColor: 'var(--warning-border)', backgroundColor: 'var(--bg-app)' }}>
          <div className="card-header" style={{ borderBottomColor: 'var(--warning-bg)', padding: '10px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--warning)', fontWeight: 600, fontSize: '12px' }}>
          ...
          <div className="card-body" style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {unconfirmed_snapshots_count > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px' }}>
                <span style={{ color: 'var(--warning)' }}>⚠️ <strong>Reconciliation Pending:</strong> End-of-day snapshots are not confirmed. You must reconcile and confirm the day's dispatches.</span>
      ...
      <div className="dashboard-grid">
        ...
        <div className="inventory-bar-container card">
          <div className="card-header">
            ...
            <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 500 }}>System Date: {formatDate(system_date)}</span>
          </div>
              ...
              {inventory_statuses.map(stat => (
                <div key={stat.product_type} className="inventory-gauge-card">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'monospace' }}>{stat.stock.toFixed(1)} <span style={{ fontSize: '11px', fontWeight: 500, color: 'var(--text-muted)' }}>MT</span></span>
                    <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Min. Safe: {stat.threshold} MT</span>
                  </div>
        ...
        <div className="stat-row">
          <div className="stat-card">
            <div>
              ...
              <div className="stat-val" style={{ color: 'var(--success)' }}>{open_po_tier_counts.A}</div>
            </div>
            <span style={{ fontSize: '11px', color: 'var(--success)', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => onNavigate('po')}>
          ...
          <div className="stat-card">
            <div>
              ...
              <div className="stat-val" style={{ color: 'var(--info)' }}>{open_po_tier_counts.B}</div>
            </div>
            <span style={{ fontSize: '11px', color: 'var(--info)', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => onNavigate('po')}>
          ...
          <div className="stat-card">
            <div>
              ...
              <div className="stat-val" style={{ color: 'var(--text-primary)' }}>{open_po_tier_counts.C}</div>
            </div>
            <span style={{ fontSize: '11px', color: 'var(--text-primary)', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => onNavigate('po')}>
              View Queue <ArrowRight size={12} />
            </span>
          </div>
          ...
          <div className="stat-card" style={{ borderLeft: '4px solid #1C6BF4' }}>
            <div>
              ...
              <div className="stat-val" style={{ fontSize: '18px', display: 'flex', alignItems: 'center', gap: '6px', color: unconfirmed_snapshots_count === 0 ? 'var(--success)' : 'var(--warning)', marginTop: '6px' }}>
                {unconfirmed_snapshots_count === 0 ? (
                  <>
                    <CheckCircle size={18} />
                    <span>Confirmed</span>
            ...
            <span style={{ fontSize: '11px', color: 'var(--primary-blue)', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => onNavigate('inventory')}>
        ...
        <div className="card chart-card">
          ...
          <div className="card-body">
            <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
              ...
              <div style={{ flex: 1.2, backgroundColor: 'var(--bg-subtle)', border: '1px solid #E2E8F0', padding: '16px', borderRadius: '4px', fontSize: '13px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '12px', fontFamily: 'monospace' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <strong style={{ color: 'var(--danger)' }}>{inventory_statuses.find(i => i.product_type === selectedProductChart)?.threshold} MT</strong>
                  </div>
                </div>
              ...
              <table className="sap-table">
                ...
                <tbody>
                    ...
                    <tr key={`${po.po_id}-${po.product_type}`}>
                      ...
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                          <span className="badge" style={{ backgroundColor: po.score >= 100 ? 'var(--success-bg)' : po.score >= 60 ? 'var(--info-bg)' : 'var(--bg-subtle)', color: po.score >= 100 ? 'var(--success)' : po.score >= 60 ? 'var(--info)' : 'var(--text-primary)', fontWeight: 700 }}>
                            {po.score} pts
                          </span>
                          {po.commitment_points > 0 && (
                            <span style={{ fontSize: '8px', color: 'var(--warning)', fontWeight: 700 }}>⚡ +{po.commitment_points} urgency</span>
                          )}
                        </div>
                      </td>
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                          ...
                          {po.commitment_status && (
                            <span style={{
                              ...
                              background: po.commitment_status === 'Missed' ? 'var(--danger-bg)' : po.commitment_status === 'Pending' ? 'var(--info-bg)' : 'var(--warning-bg)',
                              color: po.commitment_status === 'Missed' ? 'var(--danger)' : po.commitment_status === 'Pending' ? 'var(--info)' : 'var(--warning)',
                  ...
                  {(!data.actionable_po_pool || data.actionable_po_pool.length === 0) && (
                    <tr>
                      <td colSpan="8" style={{ textAlign: 'center', padding: '16px', color: 'var(--text-muted)' }}>No pending orders requiring dispatch.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="card-body" style={{ padding: '12px 16px' }}>
              {(!relationship_risk_companies || relationship_risk_companies.length === 0) ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {relationship_risk_companies.map((c, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: 'var(--danger-bg)', border: '1px solid #FECACA', borderRadius: '4px', fontSize: '12px' }}>
                      <div>
                        <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{c.name} <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(Tier {c.tier})</span></div>
                        <div style={{ fontSize: '10px', color: 'var(--text-disabled)', marginTop: '2px' }}>Health Score: {c.commitment_health_score !== null ? Math.round(c.commitment_health_score) + '%' : 'N/A'}</div>
                      </div>
                      <div style={{ width: '60px', height: '5px', borderRadius: '3px', background: 'var(--border-color)', overflow: 'hidden', flexShrink: 0 }}>
                        <div style={{
                          width: `${Math.min(100, c.commitment_health_score || 0)}%`,
                          height: '100%', borderRadius: '3px',
                          background: (c.commitment_health_score || 0) >= 60 ? 'var(--success)' : 'var(--danger)'
                        }} />
                      </div>
                    </div>
                  ))}
              ...
              {(!missed_commitments || missed_commitments.length === 0) ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {(missed_commitments || []).slice(0, 4).map((m, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: 'var(--danger-bg)', border: '1px solid #FECACA', borderRadius: '4px', fontSize: '11px' }}>
                      <div>
                        <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'monospace' }}>{m.po_id}</div>
                        <div style={{ color: 'var(--text-muted)', marginTop: '1px' }}>{m.company_name} — <span style={{ color: 'var(--danger)', fontWeight: 600 }}>Due: {formatDate(m.committed_dispatch_date)}</span></div>
                      </div>
                      <span style={{ padding: '2px 6px', background: 'var(--danger-bg)', color: 'var(--danger)', borderRadius: '3px', fontSize: '9px', fontWeight: 700, border: '1px solid #FECACA' }}>
                        MISSED
                      </span>
                    </div>
                  ))}
                  {missed_commitments.length > 4 && (
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)', textAlign: 'center', marginTop: '4px' }}>+{missed_commitments.length - 4} more — <span style={{ color: 'var(--info)', cursor: 'pointer' }} onClick={() => onNavigate('commitment-health')}>view all</span></div>
                  )}
                </div>
              )}
            </div>
